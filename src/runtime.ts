...
export class Intercom {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.endpoint = await (this.options.listen ?? listen)(own?.port, m => this.receive(m));
      if (!previous) await this.store.initialize(this.id(), this.endpoint.port);
      const config = await this.store.read();
      const me = config.agents.find(a => a.sessionId === this.id());
      if (me?.coordinator) {
        await this.store.update(this.id(), c => { requireCoordinator(c, this.id()).port = this.endpoint!.port; });
        try { await this.reload(); } catch (e) { this.host.notify(`Name/responsibility synchronization failed: ${String(e)}`); }
        this.host.notify(`Intercom Coordinator ready at 127.0.0.1:${this.endpoint.port}; waiting for user input.`);
      } else {
        if (me) { try { await this.reload(); } catch (e) { this.host.notify(`Name/responsibility synchronization failed: ${String(e)}`); } }
        try {
          if (me) await this.report();
          else await this.transmit(coordinator(config).name, 'registration', { port: this.endpoint.port, projectDirectory: await directory(this.store.root, this.host.cwd) });
        } catch (e) { this.host.notify(`Coordinator unreachable/registration failed; worker remains reachable: ${String(e)}. No retry. Anonymous registration repeat is an unresolved contract; use explicit Pi /reload to restart this extension, not report_status.`); }
  ...
  async receive(message: Envelope): Promise<void> {
    ...
    if (message.kind === 'request_status') {
      ...
      setImmediate(() => { if (this.active) void this.report().catch(e => this.host.notify(`Status report failed: ${String(e)}`)); });
      return;
    }
  ...
  async tool(operation: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    if (operation === 'configure_worker') {
      await this.store.configure(id, args as unknown as Omit<Agent, 'coordinator'>);
      return { written: true, reloaded: false };
    }
    if (operation === 'set_multiplexer') {
      await this.store.update(id, c => { c.multiplexer = args.multiplexer as Config['multiplexer']; }); return { written: true };
    }
    ...
    if (operation === 'remove_worker') {
      ...
      await this.store.update(id, c => { c.agents = c.agents.filter(a => a.sessionId !== target.sessionId); });
